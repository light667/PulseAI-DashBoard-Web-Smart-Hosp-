...
if (btnGeo) {
    ...
    (async function init() {
        ...
        if (hospital) {
            ...
            // Ajout d'un bouton pour aller au dashboard
            const actionsDiv = document.querySelector('#profileForm .actions')
            if (actionsDiv && !document.getElementById('btnGoDashboard')) {
                const btnDash = document.createElement('button')
                btnDash.id = 'btnGoDashboard'
                btnDash.textContent = 'Accéder au Tableau de Bord'
                btnDash.style.backgroundColor = '#4CAF50'
                btnDash.style.marginRight = '10px'
                btnDash.onclick = () => window.location.href = 'dashboard.html'
                actionsDiv.prepend(btnDash)
            }
        }
    })()
...
btnSubmit?.addEventListener('click', async () => {
    ...
    const { data, error } = await supabase.from('hospitals').upsert(payload).select()
